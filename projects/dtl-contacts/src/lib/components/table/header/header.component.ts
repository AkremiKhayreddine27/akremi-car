...
export class DtlTableHeaderComponent implements OnInit, AfterViewInit {
  ...
  sort(field: string, direction) {
    ...
    this.store.dispatch(
      new fromStore.LoadContacts({ filters: [], andOperator: true }, this.sorts)
    );
  }
    ...
    this.sorts = [
      {
        field: "emails",
        direction: this.sortDirection,
        compare: (direction: any, a: Email[], b: Email[]) => {
    ...
    this.sorts = [
      {
        field: "phoneNumbers",
        direction: this.sortDirection,
        compare: (direction: any, a: PhoneNumber[], b: PhoneNumber[]) => {
          const first = a.find(item => {
            return item.primary;
          }).value;
          const second = b.find(item => {
    ...
    this.sorts = [
      {
        field: "groups",
        direction: this.sortDirection,
        compare: (direction: any, a: Group[], b: Group[]) => {
